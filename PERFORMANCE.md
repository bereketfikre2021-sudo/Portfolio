...
- **Main-thread work**: AOS init runs in `requestIdleCallback` (or `setTimeout` fallback) so it doesn’t block LCP. One shared scroll listener in `ScrollContext` drives both scroll progress and scroll-to-top to cut duplicate work.
- **LCP**: Hero image is preloaded with `fetchpriority="high"`, has explicit dimensions, and critical CSS includes `.hero-image-container { aspect-ratio: 4/5 }` to reduce layout shift.
