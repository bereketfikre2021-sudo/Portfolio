...
const BottomNav = () => {
  ...
  const navItems = [
    { href: '#home', label: 'Home', section: 'home', number: '01', icon: (
      <svg className="bottom-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>
        <polyline points="9 22 9 12 15 12 15 22"/>
      </svg>
    )},
    { href: '#about', label: 'About', section: 'about', number: '02', icon: (
      <svg className="bottom-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
        <circle cx="12" cy="7" r="4"/>
      </svg>
    )},
    { href: '#services', label: 'Services', section: 'services', number: '03', icon: (
      <svg className="bottom-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M12 2L2 7l10 5 10-5-10-5z"/>
        <path d="M2 17l10 5 10-5M2 12l10 5 10-5"/>
      </svg>
    )},
    { href: '#portfolio', label: 'Portfolio', section: 'portfolio', number: '04', icon: (
      <svg className="bottom-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {navItems.map((item) => (
        ...
        >
          ...
          <span className="bottom-nav-label">
            <span className="bottom-nav-number">{item.number}</span> {item.label}
          </span>
        </a>
      ))}
    </nav>
