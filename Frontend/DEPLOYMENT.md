...
2. **Asset paths**: Vite serves `public/` at the site root; image paths like `/assets/...` resolve correctly in production
3. **Build Process**: `npm run build` (Vite) copies `public/` into `build/` and bundles JS/CSS into `build/assets/`
...
- **Build Command**: `npm ci && npm run build` (Vite → `build/`)
